...
export default {
  title: 'ChartCanvas examples',
  component: ChartCanvas,
  argTypes: {
  },
} as Meta<typeof ChartCanvas>;


export const ChartBasicDemo = () => {
  const numPts = 100;
  const numTheta = 360;
  const numLines = 5;
  const scaleX = 1000;
  ...
  const [ptsX, ptsY, ptsSize] = React.useMemo<
  ...
  const [lineX01, lineY01, color01] = React.useMemo<
  ...
  return (
    <div style={{"height":"70vh"}}>
      ...
      >
        <ChartLineGroup lines={lines} width={4} color={0xff0000ff} zOrder={2} />
