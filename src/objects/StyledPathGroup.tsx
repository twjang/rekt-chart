...
export const ChartStyledPathGroup = (prop: ChartStyledPathGroupProp) => {
  ...
  const [numPoints, numTriangles] = React.useMemo<[number, number]>(() => {
    ...
    prop.paths.forEach((path) => {
      if (path.x.length < 2 || path.x.length !== path.y.length) return;
  ...
  >(() => {
    ...
    prop.paths.forEach((path) => {
      ...
      const ptsIdx2 = ptsIdx * 2;
      const ptsIdx4 = ptsIdx * 4;
      const ptsIdx6 = ptsIdx * 6;
      const ptsIdx8 = ptsIdx * 8;
      ...
      if (typeof path.color === 'number' || path.color === undefined) {
        ...
        if (path.x.length * 4 !== path.color.length) {
          console.warn(
            'errornous color (x.length * 4 !== color.length) detected'
          );
          return;
        }
      ...
      const convertedWidth = Math.floor(
        Math.max(1, Math.min(255.0, path.width || prop.width)) * 256
      );
      idx2 = 0;
      for (let i = 0; i < path.x.length; i++) {
  ...
  return (
    <mesh ref={meshRef}>
      <bufferGeometry>
        ...
        />
        <bufferAttribute
          attach="attributes-tgntA"
          count={vertTgntA.length / 2}
          array={vertTgntA}
          itemSize={2}
          usage={THREE.DynamicDrawUsage}
        />
        <bufferAttribute
          attach="attributes-tgntB"
          count={vertTgntB.length / 2}
          array={vertTgntB}
          itemSize={2}
          usage={THREE.DynamicDrawUsage}
        />
